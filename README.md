...
Early scaffold. Ten crates compile and pass 30 tests; no native protocol,
backend, or GPU wiring yet. The compositor's frame pipeline runs end to end
down to a `FrameReport` (damage-driven, adaptive per-output scheduling), and
a full Wayland client lifecycle (connect → create surface → attach buffer →
damage → commit → disconnect with no orphaned surfaces) runs through the
compatibility layer against that same pipeline. Presenting a `FrameReport`
through SHER-Graphics is the next real milestone, not yet done.
...
recording, animation, session, XWayland compatibility, headless mode,
AI/agent APIs, diagnostics, configuration.
